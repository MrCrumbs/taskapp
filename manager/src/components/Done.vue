        ...
        <v-col cols="12">
          <v-data-table :headers="columns" :items="newTasks" class="elevation-1" :search="search" :footer-props="{'itemsPerPageText':'שורות לעמוד'}">
            ...
            <template v-slot:item.status="{ item }">
              ...
              <!-- <v-select v-model="item.status" v-on:change="updateTaskStatus($event,item)" :items="['חדש', 'התקלה לא ברורה', 'התקלה לא נמצאה', 'טיפול מתמשך', 'בוצע']" :key="'status_select_list'+item._id"  label="סטאטוס" dense solo></v-select> -->
